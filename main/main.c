...
static const char *TAG = "LYRAT CHAT";
...
void lyrat_task(void *pv)
{
   ...
   periph_wifi_cfg_t wifi_cfg = {
        ...
        .reconnect_timeout_ms = 10000,  // Ejemplo de tiempo de espera de 10 segundos
...
ESP_LOGI(TAG, "[ 1 ] button id=%d", get_input_rec_id());
    ...
    while (1) {
        ...
        if (original_text == NULL) {
            ...
            ESP_LOGI(TAG, "[ * ] original_text: %s", original_text);
            if(strstr(original_text, "temperatura")){
                typeData = 1;
            }
            else if(strstr(original_text, "humedad")){
                typeData = 2;
            }
            else if(strstr(original_text, "calidad de aire")){
                typeData = 3;
            }
            ESP_LOGI(TAG, "[ * ] sensor tipo : %d", typeData);

            text = send_text(typeData);
            typeData = 0;
...
void app_main(void)
{
    ...
    xTaskCreate(lyrat_task, "lyrat_task", 2 * 4096, NULL, 5, NULL);
}
